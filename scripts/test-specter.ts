...
async function main() {
  ...
  check(
    targetPreviewGhost.includes("CURSOR_HOTSPOT") &&
      targetPreviewGhost.includes("left: `${percentX}%`") &&
      targetPreviewGhost.includes(
        "translate(-${CURSOR_HOTSPOT.x}px, -${CURSOR_HOTSPOT.y}px)",
      ),
    "TargetPreviewGhost uses percent positioning with hotspot correction",
  );
  check(
  ...
  check(
    ghostCursor.includes("step.viewportX ?? step.x") &&
      ...
      ghostCursor.includes("left: `${percentX}%`") &&
      ghostCursor.includes(
        "translate(-${CURSOR_HOTSPOT.x}px, -${CURSOR_HOTSPOT.y}px)",
      ) &&
  ...
  check(
    replayAuto.includes("clickRealMouse") &&
      autoBody.includes("clickStepViewport") &&
  ...
  check(
    ghostActionPlayerBody.includes("useGhostSpring") &&
      ghostActionPlayerBody.includes("ghost-travel-trail"),
    "GhostActionPlayer uses spring animation hook and trail",
  );
  check(
    readFile("src/renderer/overlay/useGhostSpring.ts").includes("isSettled") &&
      ghostActionPlayerBody.includes("useGhostSpring"),
    "live ghost uses useGhostSpring travel via GhostActionPlayer",
  );
  check(
