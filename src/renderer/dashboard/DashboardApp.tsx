...
export const DashboardApp: React.FC = () => {
  const [tab, setTab] = useState<"memories" | "progress" | "docs">("memories");
  ...
  const loadDoc = useCallback(async (id: string) => {
    try {
      const res = await window.dashboard.getDoc(id);
      if (res?.ok && res.body) {
        setSelectedDocId(id);
        setDocTitle(res.title || id);
        setDocBody(res.body);
      }
    } catch (error) {
      console.error("[DASHBOARD] getDoc failed", error);
    }
  }, []);

  const refresh = useCallback(async () => {
    try {
      const [mem, prog, docList] = await Promise.all([
        window.dashboard.listMemories(),
        window.dashboard.getProgress(),
        window.dashboard.listDocs(),
      ]);
      if (mem?.ok) setMemories(mem.memories);
      if (prog?.ok) setApps(prog.apps);
      if (docList?.ok) {
        setDocs(docList.docs);
        if (tab === "docs" && !selectedDocId && docList.docs.length > 0) {
          void loadDoc(docList.docs[0].id);
        }
      }
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: colors.dim }}>
              {tab === "docs"
                ? "Project docs — computer use, testing, runbooks"
            ...
            >
              All on this Mac. Nothing leaves your computer.
            </div>
          </div>
        ...
        {tab === "memories" && (
          <>
            ...
            {filteredMemories.length === 0 && (
              ...
              >
                No memories yet. Summon Specter (double-tap Shift), walk through
                a task, and it appears here automatically.
              </div>
            )}
        ...
        {tab === "docs" && (
          ...
          >
                ...
                {filteredDocs.map((d) => (
                  <button
                    ...
                    style={{
                      textAlign: "left",
                      background:
                        selectedDocId === d.id ? colors.panelHover : colors.panel,
                      border: `1px solid ${
                        selectedDocId === d.id ? colors.accent + "66" : colors.border
                      }`,
                      borderRadius: 8,
                      color: colors.text,
                      padding: "8px 10px",
            ...
            >
              {docBody ? (
                <>
                  <h2 style={{ fontSize: 16, margin: "0 0 12px" }}>{docTitle}</h2>
                  <pre
                    style={{
                      margin: 0,
                      fontSize: 12,
                ...
                <div style={{ color: colors.dim, fontSize: 13, padding: 40 }}>
