...
// Auto-compile the most recently saved session to the wiki so the Memory
// dashboard shows real sessions without any manual button press.
function autoCompileToWiki(graph: any, appName: string): void {
  if (!graph?.sessions?.length) return;
  ...
  try {
    ...
    safeLog("[AutoCompile] session compiled to wiki", {
      sessionId: session.id,
      pages: pages.length,
    });
    void import("./hubSync").then((m) => m.pushJourneySnapshot(graph, appName));
  } catch (e) {
    safeError("[AutoCompile] failed", { error: e });
  }
}
...
function createRealAppStep(target: any, source: "vision" | "manual"): Step {
  ...
  const axApp =
    typeof target?.axApp === "string" && target.axApp.trim()
      ...
      : undefined;
  const axDumpIndex =
    typeof target?.axDumpIndex === "number" && Number.isFinite(target.axDumpIndex)
      ? target.axDumpIndex
      : undefined;
  const axDumpApp =
    typeof target?.axDumpApp === "string" && target.axDumpApp.trim()
      ? target.axDumpApp.trim()
      : undefined;
...
app.whenReady().then(async () => {
  ...
  ipcMain.handle("journey:publish", async (_event, skillId: string) => {
    const { publishSkillToHub } = await import("./hubSync");
    const ok = await publishSkillToHub(String(skillId || ""));
    return { ok };
  });

  ipcMain.handle(
    "tavus:uploadPhoto",
    async (_event, payload: { imageBase64?: string; mimeType?: string }) => {
      const { tavusUploadPhoto } = await import("./tavusHub");
      return tavusUploadPhoto(
  ...
  ipcMain.handle(
    "tavus:createReplica",
    async (_event, uploadUrl: string) => {
      const { tavusCreateReplica } = await import("./tavusHub");
      return tavusCreateReplica(String(uploadUrl || ""));
    },
  );
  ipcMain.handle("tavus:getReplicaStatus", async () => {
  ...
  ipcMain.handle(
    ...
    async (_event, nodeId, steps, appName = DEFAULT_APP_NAME) => {
      ...
      saveGraph(graph);
      autoCompileToWiki(graph, appName);
      return graph;
    },
  );
  ...
  ipcMain.handle(
    ...
    async (_event, sessionId, appName) => {
      ...
      const pages = compileSessionToWiki(session, appName);
      const wikiRoot = process.env.GHOSTWIKI_WIKI_ROOT || "./demo-workflows/event-recap/wiki";
