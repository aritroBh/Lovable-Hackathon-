...
export class NvidiaVisionProvider implements VisionProvider {
  ...
  async analyze(input: VisionAnalyzeInput): Promise<VisionAnalyzeResult> {
    ...
    // Per-call overrides take precedence; existing callers pass neither, so the
    // env-driven defaults (Maverick + NVIDIA_API_KEY) are unchanged for them.
    ...
    try {
      safeLog(`[VISION][NVIDIA] Calling ${model}...`, {
        task: input.task,
      });
      ...
      return {
        ...
        // SoM grounding parses chosenIndex out of the raw model text, so it must
        // always be available for that task (not only outside production).
        rawText:
          input.task === "set_of_mark" || process.env.NODE_ENV !== "production"
            ? content
            : undefined,
      };
    } catch (error: any) {
