...
export interface ParsedSetOfMark {
  chosenIndex: number;
  confidence: number;
  reasoning?: string;
}
...
/**
 * Parse a Set-of-Mark grounding response: { chosenIndex, confidence, reasoning }.
 *
 * Deliberately does NOT route through validateVisionOutput — that builds a fixed
 * {summary, elements, ...} shape and drops unknown fields like chosenIndex.
 * Reuses the extractJsonObject brace-scan to tolerate stray prose around the JSON.
 *
 * Never throws: on any parse/shape failure returns null so the caller can fall
 * back to "ask the user" instead of crashing the resolve pipeline.
 */
export function parseSetOfMarkJson(text: string): ParsedSetOfMark | null {
  ...
  const rawIndex = obj.chosenIndex;
  const index =
  ...
  const chosenIndex = Math.trunc(index);
  ...
  return { chosenIndex, confidence, reasoning };
}
