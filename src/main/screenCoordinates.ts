...
>(target: T, captureMeta: CaptureFrameMeta): T & ViewportPercentTarget {
  ...
  // Only skip remap for hand-placed or already viewport-normalized targets.
  const alreadyViewport =
    sourceFrame === "viewport" &&
    target.coordinateFrame === "viewport" &&
    (!target.rawTarget || target.rawTarget.coordinateFrame === "viewport");

  if (sourceFrame === "manual" || alreadyViewport) {
...
export function normalizePracticeWindowTargetToViewportPercent(
  ...
  // ponytail: accept 0–1 fractions or 0–100 percents from callers
  const fracX = target.x > 1 ? target.x / 100 : target.x;
  const fracY = target.y > 1 ? target.y / 100 : target.y;
  const rawX = clampPercent(fracX * 100);
  const rawY = clampPercent(fracY * 100);
  const logicalX = bounds.x + bounds.width * fracX;
  const logicalY = bounds.y + bounds.height * fracY;
...
export function logicalPointToPercent(
  ...
  return {
    x: clampPercent(((x - b.x) / b.width) * 100),
    y: clampPercent(((y - b.y) / b.height) * 100),
  };
}
